...
# 固定的 Dropbox Excel 路徑
DROPBOX_EXCEL_PATH = "/eln-bot/(發LINE版)ELN 自動追蹤KO、KI、到期狀況v2.xlsx"
...
def download_from_dropbox(dbx_token: str, dropbox_path: str) -> str:
    """從 Dropbox 下載 Excel，回傳本地暫存路徑"""
    dbx = dropbox.Dropbox(dbx_token)
    _, response = dbx.files_download(dropbox_path)
    tmp = tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False)
    tmp.write(response.content)
    tmp.close()
    print(f"Downloaded from Dropbox: {dropbox_path} → {tmp.name}")
    return tmp.name
...
def main():
    ...
    # 環境變數
    dropbox_token = get_env("DROPBOX_TOKEN")
    ...
    # chat_key 設定為你的個人 user_id
    chat_key = f"user:{user_id}"

    try:
        # 1. 從 Dropbox 下載 Excel
        print("Downloading Excel from Dropbox...")
        excel_path = download_from_dropbox(dropbox_token, DROPBOX_EXCEL_PATH)
        ...
        save_result_to_db(engine, chat_key, summary, top5_lines, detail_map, agent_name_map)
        ...
        # 5. 把個別通知存進 pending 表，等你確認後才發
        individual_messages = out.get("individual_messages", [])
        if individual_messages:
            ...
            line_bot_api.push_message(user_id, TextSendMessage(text="✅ 今日無需通知理專"))
        ...
        try:
            line_bot_api.push_message(user_id, TextSendMessage(text=f"⚠️ ELN 自動追蹤失敗：{e}"))
        except Exception:
            pass

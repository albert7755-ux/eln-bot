...
def generate_bond_commentary(snapshot_text: str) -> str:
    ...
    prompt = (
        ...
        "2.【殖利率動向解讀】★版型硬性規定,務必照抄這個骨架:\n"
        "   第1行:一句總述,點出昨晚有幾件事推動利率。例:「昨晚有三個消息推升利率」。\n"
        "   接著:每一件事各自獨立成一段,一段只講一件事,不要把兩件事寫在同一段。\n"
        "   每段格式固定為 ——\n"
        "     📝 事件短標(12字以內)：具體內容(40~60字,要有數字、機構名或人名)\n"
        "   事件最多3個、最少1個;沒有第3件就只寫2段,不要硬湊。\n"
        "   最後一段(不加任何符號、與上面空一行):寫各天期怎麼反應,60字以內,"
        "短天期與長天期分開講。\n"
        "   【常見錯誤】不要先用一段話把三件事講完、再用另一段話把三件事的細節重講一次——"
        "那樣讀者要來回對照。一件事就從頭到尾講完,再講下一件。\n"
        "   整段(含所有小段)合計不超過350字。\n"
        "   內容要求:第一件事必須是『昨晚殖利率變動最直接的觸發事件』"
        "(引用具體新聞與數字,例如油價跌幅、數據結果、官員談話內容);若確實找不到明確觸發事件,"
        "就誠實說明市場在等待什麼,不要拿結構性題材硬湊。說明美債各天期為什麼這樣動,"
        ...
        f"3.【今日專題】用80-120字寫一則小專題,今天的主題是:{topic}。"
        "只挑1個最重要的事件講,寧短勿長。\n"
        "   【嚴禁複述數據區】上方數據區已經列出的數字——各天期殖利率與bp變動、"
        "TLT/LQD/HYG 的漲跌幅、油價漲跌幅——一律不要在專題裡再講一次。"
        "專題要提供數據區沒有的新資訊(例如發行量、利差水準、政策進度、供需結構),"
        "否則整段沒有存在價值。\n"
        "4.【今日操作思維】上限200字,寫給「我們」的觀察與提醒,不是判斷與指令。\n"
        "   版型:分成2~3個短段,段與段之間空一行,一段講一個重點,每段1~2句。"
        "不要寫成一大塊。不加任何符號。\n"
        "   【嚴禁教學口吻】不要寫「這個對比值得在跟客戶溝通時提出來」"
        "「這兩件事要分開說清楚」「可以這樣跟客戶講」這類指導讀者怎麼做的句子。"
        "把觀察本身寫完就停,讀者自己會判斷。\n"
        ...
        "【符號用量】全篇唯一允許的符號是【殖利率動向解讀】裡每個觸發事件開頭的 📝。"
        "除此之外,四個段落的內文一律不得出現任何 emoji、圖示、• ・ ▸ ◆ ※ 等符號,"
        "也不要用數字編號(1. 2. 3.)或破折號當條列。"
        "符號少才有辨識度,滿篇符號跟只貼新聞標題的群組沒有差別。\n"
    )
...
def _shape_problems(secs: dict):
    """
    檢查版型有沒有跑掉。AI 常見的漂移是:
    「殖利率動向解讀」寫回一大塊、「操作思維」寫成一長段、或整段爆字數。
    回傳問題清單(空的代表版型OK),用來決定要不要重生一次。
    """
    probs = []
    y = (secs.get("殖利率動向解讀") or "").strip()
    if y:
        if "📝" not in y and len(y) > 180:
            probs.append("殖利率動向解讀沒有分事件條列,寫成一大塊")
        if len(y) > 420:
            probs.append(f"殖利率動向解讀過長({len(y)}字,上限350)")
    a = (secs.get("今日操作思維") or "").strip()
    if a:
        if len(a) > 260:
            probs.append(f"今日操作思維過長({len(a)}字,上限200)")
        if len(a) > 160 and "\n" not in a:
            probs.append("今日操作思維沒有分段")
    t = (secs.get("今日專題") or "").strip()
    if t and len(t) > 190:
        probs.append(f"今日專題過長({len(t)}字,上限120)")
    return probs
...
def build_final_bond_report(data: dict) -> str:
    ...
    shape = _shape_problems(secs)
    if missing or shape:
        print(f"[BondDaily] 段落缺少 {missing}、版型問題 {shape},重試一次。"
              f"原始輸出前400字:\n{(commentary or '')[:400]}")
        try:
            ...
            if shape:
                hint += ("\n【上一次的版型錯誤】" + "；".join(shape) +
                         "。【殖利率動向解讀】務必照這個骨架:第1行一句總述,"
                         "接著每件事一段、以『📝 事件短標：內容』開頭、一段只講一件事,"
                         "最後一段不加符號寫各天期反應;全段不超過350字。"
                         "【今日操作思維】分2~3個短段、段間空一行、合計不超過200字。")
            commentary2 = generate_bond_commentary(snapshot + hint)
            secs2 = parse_sections(commentary2)
            better = (sum(1 for k in need if secs2.get(k)) > sum(1 for k in need if secs.get(k))
                      or len(_shape_problems(secs2)) < len(shape))
            if better:
                secs = secs2
        except Exception as e:

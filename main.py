...
def save_article_text(ck: str, content: str) -> str:
    ...
    title_line = next((l for l in summary.splitlines() if l.startswith("標題：")), "")
    title = title_line.replace("標題：", "").strip() or content[:30]
    source_type = "url" if content.startswith("http") else "text"
    with engine.begin() as conn:
        conn.execute(text("""
        INSERT INTO articles (title, content, summary, source_type, is_read)
        VALUES (:t, :c, :s, :st, FALSE)
        """), {"t": title, "c": content[:5000], "s": summary, "st": source_type})
    return summary

def save_article_image(image_data: bytes, message_id: str) -> str:
    """儲存圖片，用 Claude Vision 看圖摘要"""
    ...
    summary = (resp.content[0].text or "").strip()
    title_line = next((l for l in summary.splitlines() if l.startswith("標題：")), "")
    title = title_line.replace("標題：", "").strip() or "圖片文章"
    with engine.begin() as conn:
        conn.execute(text("""
        INSERT INTO articles (title, content, summary, source_type, image_url, is_read)
        VALUES (:t, :c, :s, 'image', :img, FALSE)
        """), {
            "t": title,
            "c": "（圖片）",
            "s": summary,
